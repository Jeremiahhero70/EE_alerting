...
The system prevents duplicate email alerts using a **smart hash-based cache**:

#### How It Works

1. **Alert Fingerprinting**: For each alert, the system creates a unique identifier based on:
   ```
   Rule ID | Agent ID → MD5 Hash
   ```
   **Note**: Timestamp is NOT included - same rule on same agent = always deduplicated.
   
   Example:
   ```
   Foreign login from user@company.com, IP 203.0.113.45 at 14:23:15
   → Rule 5101 | Agent 002 → Hash ABC123
   
   Same user, same IP, 2 hours later at 16:45:30
   → Rule 5101 | Agent 002 → Hash ABC123 (SAME HASH - email skipped)
   ```

2. **Cache Checking**: Before sending an email:
   - Generates hash from the alert (Rule ID + Agent ID)
   - Checks if hash exists in today's cache file
   - If found → **Skip email** (already alerted on this today)
...
3. **Daily Rotation**: Cache files reset each day
   ...
   - New day = new file and new alerting cycle

#### Why This Approach?

- **No alert fatigue**: Same issue = 1 email per day, not multiple
- **Focused alerts**: Only new problems trigger notifications
- **Recurring issues still caught**: Tomorrow is a new cache file, so recurring daily issues will re-trigger

#### Why Hashing?

- **Speed**: Hash comparison is instant (no data parsing)
- **Efficiency**: Small hash size vs storing full alert data
- **Accuracy**: Same rule + agent = same hash (reliable deduplication)
- **Security**: Raw credentials/IPs not exposed in cache files

#### Example Scenario

Same foreign login from one user throughout the day:
```
08:15:00 → Hash generated → Email sent ✉️  (new alert today)
08:45:00 → Hash in cache → Email skipped ⏭️
12:30:00 → Hash in cache → Email skipped ⏭️
15:20:00 → Hash in cache → Email skipped ⏭️
18:00:00 → Hash in cache → Email skipped ⏭️

Next day (tomorrow):
08:10:00 → NEW cache file → Email sent ✉️  (recurring issue detected)
```

**Result**: 1 email per day for recurring issues, preventing alert fatigue!
